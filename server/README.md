...
| `downloads`           | object  | 各平台下载地址（arm64/arm32/x86_64/all，https） | `{"arm64":"https://.../notice_arm64_1.5.73.apk",...}` |
| `fileSizes`           | object  | 各平台文件大小（字节）    | `{"arm64":27349386,...}`         |
| `sha256`              | object  | 各平台安装包 sha256（v1.5.73+ 传输层校验，App 下载后安装前比对） | `{"arm64":"63c6...09b",...}` |
...
1. 打开管理后台：`https://your-domain.com/admin.html`
2. 输入管理员 Token
3. 点击「首次登录 - 设置二步验证」
4. 扫描二维码到认证应用
5. 输入生成的 6 位验证码
