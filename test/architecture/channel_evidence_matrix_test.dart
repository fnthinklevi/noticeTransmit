...
void main() {
  ...
  final matrix =
      jsonDecode(
            File(
              '$root/test/evidence/channel_availability_matrix.json',
            ).readAsStringSync(),
          )
          as Map<String, dynamic>;
  final rows = (matrix['types'] as List<Object?>).cast<Map<String, dynamic>>();
  ...
  final golden =
      jsonDecode(
            File(
              '$root/android/app/src/test/resources/channel_behavior_golden.json',
            ).readAsStringSync(),
          )
          as Map<String, dynamic>;
  final payloadKeys = (golden['payloads'] as Map<String, dynamic>).keys.toSet();
  ...
  group('证据矩阵的结构', () {
    ...
    test('golden 指针：说"有四个场景"就必须真有四个场景键', () {
      for (final r in rows.where(
        (r) => (r['payloadEvidence'] as String).startsWith('golden'),
      )) {
    ...
    test('未盖章的缺口是收缩棘轮：只许变少，变少要同时改数字', () {
      final pending = rows
          .where((r) => (r['payloadEvidence'] as String).startsWith('todo:'))
          .toList();
    ...
    test('「真发一次」的章必须带日期/构建号，且总数与 ratchet 一致', () {
      ...
      print(
        ...
        '（${unwitnessed.length}/${rows.length}）',
      );
    });
  });
}
