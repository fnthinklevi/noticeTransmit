...
void main() {
  ...
  group('集成冒烟测试自身的前提', () {
    test('集成测试不得再自带描述符字面量（只用共用的原生载荷桩）', () {
      // 两份手抄桩各撞过一次静默降级（T08-B 载荷形状、T08-C2 缺 email 族）：
      // 服务认不下载荷时保留旧缓存，表现只是"表单少东西"，单测全绿。
      // 现在桩只有 integration_test/support/native_payload_stubs.dart 一份，
      // 它与导出快照的逐字段比对在 native_payload_stub_test.dart。
      for (final rel in const [
        ...
        expect(
          src,
          contains("channelDescriptorsStub(["),
          reason: '$rel 不再经共用桩取描述符：它自己抄了一份，就会漂移',
        );
        expect(
          src,
          isNot(contains("'family':")),
          reason: "$rel 里又出现了手写的描述符字面量（'family': 键）",
        );
      }
    });
