...
[![Version](https://badgen.net/badge/Version/1.5.67/007AFF?icon=android)](https://github.com/fnthinklevi/noticeTransmit/releases)
...
- 🧠 **Rule Engine** - Visual configuration of notification rules, support condition combination (IF) and action configuration (THEN), first-time entry provides feature guide, built-in default rules including verification code priority push, marketing ad blocking, night do not disturb and app notification aggregation (ready out of the box; missing rules are filled in automatically on upgrade)
- 📦 **App Notification Aggregation (v1.5.67)** - Notifications from the same app are merged into a single push within a window (60s by default, configurable, 5s minimum), greatly reducing message floods; rule conditions support the `*` wildcard to match any app; while merging, the foreground notification shows the pending list with countdown; whether aggregation succeeds or fails, member content is always saved to push history first, and the merged summary shows the real delivery status
