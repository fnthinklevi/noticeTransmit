...
[![Version](https://badgen.net/badge/Version/1.5.68/007AFF?icon=android)](https://github.com/fnthinklevi/noticeTransmit/releases)
...
- 📦 **App Notification Aggregation (v1.5.68)** - Notifications from the same app are merged into a single push within a window (60s by default, configurable, 5s minimum), greatly reducing message floods; a single message during the window is sent as a normal push; rule conditions support the `*` wildcard to match any app; while merging, the foreground notification shows the pending list with countdown; whether aggregation succeeds or fails, member content is always saved to push history first, and each member record is labeled with its real delivery status
