...
通知推送助手是一款注重**隐私与安全**的 Android 通知转发工具，基于 Flutter 开发。它监听系统通知栏消息，通过用户自配的 Webhook 实时推送到企业微信、钉钉、飞书等平台。

**核心安全设计：**
- 🔐 **全本地处理**：通知、短信、通讯录仅在设备本地解析与推送，**不上传至任何服务器**
- 🔑 **密钥加密存储**：Webhook 地址使用 AndroidKeyStore 硬件级加密，通知历史使用 AES-256 本地加密
- 🛡️ **后台 2FA 保护**：管理后台支持 TOTP 二步验证 + bcrypt 哈希 + IP 智能封锁
- 📊 **最小数据采集**：仅通过腾讯 Bugly 收集崩溃堆栈、设备型号、系统版本用于修复问题，不采集任何个人隐私

支持多通道推送、应用筛选、关键词过滤、自定义电量提醒等功能。采用 Cupertino 设计语言，支持深色模式，开源（MIT）且完全免费。
