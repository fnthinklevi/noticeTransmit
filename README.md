...
- 🏢 **自建应用通道（企业微信 / 飞书）** - 独立于 Webhook 的「应用通道」体系：填 corpsecret / app_secret 等凭据即可让应用自身发消息（企微自建应用可定向 touser/@all，飞书可指定 chat_id/open_id）；API 地址可改为私有化部署地址；密钥加密存储，送达结果与失败重试同 Webhook 通道；**内置分步接入引导**（卡片右上角「?」查看 corpid/AgentId/Secret 或 App ID/App Secret 的获取步骤与注意事项）
