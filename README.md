...
- 💾 **配置备份与恢复（v1.5.69）** - 一键将 **11 类配置**（Webhook/邮件通道含凭据、通知规则、短信设置、应用过滤、黑白名单、电池规则与电量开关、设备名、主题/语言）加密导出为 `.nbackup` 文件（AES-256-GCM + PBKDF2 210k 迭代口令派生，文件头自描述 KDF 参数），换机/重装后选文件 + 输入口令即可还原；检测到现有配置时支持「覆盖全部 / 仅导入空缺项」冲突策略，非 https 地址自动跳过；备份格式 v2 向后兼容 v1
